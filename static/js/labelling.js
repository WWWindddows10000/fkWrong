...
const fillColor = ["rgba(205,92,92,0.5)","rgba(255,140,0,0.5)","rgba(238,232,170,0.5)",
                           "rgba(107,142,35,0.5)","rgba(34,139,34,0.5)","rgba(102,205,170,0.5)",
                           "rgba(0,139,139,0.5)","rgba(70,130,180,0.5)","rgba(135,206,250,0.5)","rgba(72,61,139,0.5)"];
...
canvas.addEventListener('mouseup', function (e) {
    if (isDrawing) {
        ...
        submitButton.boxNo = currentBoxNo - 1;
        submitButton.onclick = () => submitText(submitButton.boxNo);
...
function clearCanvas() {
 ...
 function submitText(BoxNo) {
     let box = boxes[BoxNo];
     const text = box.inputElement.value.trim();
     if (text) {
         box.text = `#${ box.boxNo } | ${text}`;
     }
     box.inputElement.style.display = 'none';
     box.submitButton.style.display = 'none';
     redrawAll();
 }
